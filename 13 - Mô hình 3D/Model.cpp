...
void MoHinh::Ve(Shader& shader, GocNhin& gocNhin)
{
	// vẽ từng các lưới
	for (unsigned int i = 0; i < mangLuoi.size(); i++)
		mangLuoi[i].Ve(shader, gocNhin, maTran[i]);
}
...
vector<float> MoHinh::getFloat(json accessor)
{
	...
	unsigned int buff = accessor.value("bufferView", 1), 
	...
	unsigned int bOffset = bView["byteOffset"], n;
	...
	unsigned int batDau = bOffset + byte, len = dem * 4 * n;
	for (unsigned int i = batDau; i < batDau + len; i)
	{
...
vector<GLuint> MoHinh::getThuTu(json accessor)
{
	vector<GLuint> tt;
	// lấy thuộc tính truy cập
	unsigned int buff = accessor.value("bufferView", 0), dem = accessor["count"],
		byte = accessor.value("byteOffset", 0), type = accessor["componentType"];

	// lấy thuộc tính bufferView
	json bView = Json["bufferViews"][buff];
	unsigned int bOffset = bView["byteOffset"], batDau = bOffset + byte;
	// lấy đúng kiểu dữ liệu cho type
	switch (type)
	{
	case 5122:
		for (unsigned int i = batDau; i < bOffset + byte + dem * 2; i)
		{
	...
	case 5123:
		...
		break;
	case 5125:
		for (unsigned int i = batDau; i < bOffset + byte + dem * 4; i)
		{
			unsigned char bytes[] = { data[i++], data[i++], data[i++], data[i++] };
			unsigned int gt;
			memcpy(&gt, bytes, sizeof(unsigned int));
			tt.push_back((GLuint)gt);
		}
		break;
	}
...
vector<HinhAnh> MoHinh::getHinhAnh()
{
	...
	for (unsigned int i = 0; i < Json["images"].size(); i++)
	{
		...
		bool b = false;
		for (unsigned int j = 0; j < tenAnh.size(); j++)
			if (tenAnh[j] == anh)
			{
		...
		// chạy ảnh phân tán
		if (anh.find("baseColor") != string::npos && !b)
		{
		...
		// chạy ảnh phản chiếu
		else if (anh.find("metallicRoughness") != string::npos && !b)
		{
...
void MoHinh::taiLuoi(unsigned int mluoi)
{
	// lấy các thứ tự điểm truy cập
	unsigned int vitri = Json["meshes"][mluoi]["primitives"][0]["attributes"]["POSITION"],
	...
	mangLuoi.push_back(Luoi(td, tt, h));
}

void MoHinh::nodeQua(unsigned int next, glm::mat4 matran)
{
	// Node hiện tại
