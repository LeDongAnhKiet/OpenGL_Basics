...
void FPS(GLFWwindow* win)
{
	...
	if (ktg >= 1.0 / 30.0)
	{
		std::string fps = std::to_string((1.0 / ktg) * dem),
			...
			tieuDe = "Loai tru thi sai - " + fps + " FPS / " + ms + " ms";
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(doRong, doCao, "Parallax Occlusion Mapping", NULL, NULL);
	if (window == NULL)
	{
	...
	GocNhin gocNhin(doRong, doCao, glm::vec3(0.0f, 0.2f, 2.0f));
	VAO vao = VAO();
	...
	{
		HinhAnh("meo.png", "phanTan", 0),
		HinhAnh("sang.png", "phanChieu", 1)
	};
	Luoi luoi(diem, chiSo, hinhAnh);
	HinhAnh chuanAnh("chuan.png", "chuan", 2);
	HinhAnh saiAnh("thisai.png", "saiVt", 3);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform1i(glGetUniformLocation(chuongTrinh.ID, "saiVt"), 2);
